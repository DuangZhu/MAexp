...
from model.invariant import SingleAgentEncoder, SingleAgent_CriticEncoder
import math
from einops.layers.torch import Rearrange
import numpy as np

class actor_il(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        self.agent_critic_encoder = SingleAgent_CriticEncoder(num_grids=64, input_dim=32+16, use_id_embedding = True, use_pos_embedding = True,use_add_id_embedding = False)
        ...
        self.vf_agent_encoder = nn.Sequential(
            nn.Flatten(),
        ...
        self.critic = [self.agent_critic_encoder]
    ...
    def value_function(self) -> TensorType: 
        assert self._feature is not None, "must call forward() first"
        B = self.features.shape[0]
        B = B/self.n_agents
        out = self.agent_critic_encoder(self._feature, self._state)
        out = self.vf_agent_encoder(out)
        if self.q_flag:
