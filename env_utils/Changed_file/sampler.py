...
def _env_runner(
    ...
    while True:
        perf_stats.iters += 1
        t0 = time.time()
        if hasattr(worker.policy_map.cache['shared_policy'].model, 'patch_embed'):
            encoder = worker.policy_map.cache['shared_policy'].model.patch_embed
        else:
            encoder = None
        # Get observations from all ready agents.
        # types: MultiEnvDict, MultiEnvDict, MultiEnvDict, MultiEnvDict, ...
        unfiltered_obs, rewards, dones, infos, off_policy_actions = \
            base_env.poll(encoder)
        ...
        t4 = time.time()
        base_env.send_actions_with_encoder(actions_to_send, encoder)
