...
class BaseEnv:
    ...
    @PublicAPI
    def send_actions_with_encoder(self, action_dict: MultiEnvDict, encoder) -> None:
        """Called to send actions back to running agents in this env.
...
class _MultiAgentEnvToBaseEnv(BaseEnv):
    ...
    @override(BaseEnv)
    def poll(self, encoder = None) -> Tuple[MultiEnvDict, MultiEnvDict, MultiEnvDict,
                            MultiEnvDict, MultiEnvDict]:
        obs, rewards, dones, infos = {}, {}, {}, {}
        for i, env_state in enumerate(self.env_states):
            obs[i], rewards[i], dones[i], infos[i] = env_state.poll(encoder)
    ...
    @override(BaseEnv)
    def send_actions_with_encoder(self, action_dict: MultiEnvDict, encoder = None) -> None:
        for env_id, agent_dict in action_dict.items():
            if env_id in self.dones:
                raise ValueError("Env {} is already done".format(env_id))
            env = self.envs[env_id]
            obs, rewards, dones, infos = env.step(agent_dict, encoder)
            assert isinstance(obs, dict), "Not a multi-agent obs"
            assert isinstance(rewards, dict), "Not a multi-agent reward"
            assert isinstance(dones, dict), "Not a multi-agent return"
            assert isinstance(infos, dict), "Not a multi-agent info"
    ...
    def try_reset(self,
                  ...
                  encoder = None) -> Optional[MultiAgentDict]:
        obs = self.env_states[env_id].reset(encoder)
        assert isinstance(obs, dict), "Not a multi-agent obs"
...
class _MultiAgentEnvState:
    ...
    def reset(self, encoder) -> MultiAgentDict:
        self.last_obs = self.env.reset(encoder)
        self.last_rewards = {}
        self.last_dones = {"__all__": False}
        self.last_infos = {}
